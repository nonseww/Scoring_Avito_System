...
class FeatureExtractor:
    ...
    Вызывается и при сборке обучающей выборки, и на инференсе - этим гарантируется,
    что модель видит одинаково устроенные данные в обоих режимах.
    """
    ...
    def __init__(self, all_items: pd.DataFrame, location_service, popularity_service):
        """all_items — объединение корпуса и объявлений train"""

        # Индексация по item_id, чтобы доставать свойства кандидатов
        # векторно через .reindex().
    ...
    def extract(
        ...
        # которых нет в таблице, ставит NaN
        meta = self.meta.reindex(candidates)

        features = {}

        # Позиции в источниках. Кодируется как словарь {item_id: место},
        ...
        # Число кандидатов ДО негативного сэмплинга. При сборке обучающей
        # выборки в extract приходит только отобранная часть (31 строка),
        # поэтому реальное число передаётся параметром. На инференсе
        # сэмплинга нет — берётся len(candidates)
        total = n_candidates_total if n_candidates_total is not None else n
        features["n_candidates"] = np.full(n, total, dtype=np.float32)
        features["query_len_tokens"] = np.full(n, len(query_text.split()), dtype=np.float32)

        # Собирается в фиксированном порядке колонок
        return pd.DataFrame(features, columns=self.FEATURE_NAMES, index=candidates)

    @staticmethod
    def _haversine(lat1, lon1, lat2, lon2):
        """Расстояние по поверхности Земли в километрах в виде вектора"""
        r = 6371.0
