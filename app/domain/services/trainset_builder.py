...
class TrainsetBuilder:
    def __init__(self, pipeline, n_negatives: int = 30, hard_ratio: float = 0.5,
        ...
        self.pipeline = pipeline # уже прошедший _setup(), со всеми сервисами
        self.n_negatives = n_negatives # сколько негативов оставлять на запрос
        self.hard_ratio = hard_ratio # доля "сложных" негативов (высоко в RRF, но не выбраны)
    ...
    def build(self, queries: pd.DataFrame, gold: dict, out_path: Path,
        ...
        Пишет частями, тк 2 млн строк в память не влезут."""
        ...
        for i, row in enumerate(queries.itertuples()):
            ...
            out = self.pipeline._run_sources(
                ...
                restrict_to=self.pipeline.bench_ids,
            ...
            if len(buffer) >= batch_size:
                ...
                print(f"  {i+1}/{len(queries)} запросов, {written} строк, "
                      f"{speed:.1f} запросов/с, осталось ~{eta:.0f} мин")
    ...
    def _sample(self, candidates: list, labels: np.ndarray, rrf_ranked: list) -> np.ndarray:
        ...
        Представляет собой смесь случайных и "сложных".
        Сложные это те, которые RRF поставил высоко,
        но пользователь не выбрал: именно их модель путает с позитивами,
        и именно на них она должна учиться"""
